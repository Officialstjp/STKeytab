...
try {
    foreach ($folder in $folders) {
        if (Test-Path $folder.Path) {
            ...
            # Handle different return types from Invoke-ScriptAnalyzer more robustly
            $resultCount = 0
            if ($null -ne $results) {
                if ($results -is [array]) {
                    $resultCount = $results.Count
                } elseif ($results.PSObject.Properties['Count']) {
                    $resultCount = $results.Count
                } else {
                    $resultCount = 1
                }
            ...
            if ($resultCount -gt 0) {
                Write-Host "Found $resultCount issue(s):" -ForegroundColor Red
    ...
    Write-Error "PSScriptAnalyzer encountered an error at line $($_.InvocationInfo.ScriptLineNumber): $_"
}
