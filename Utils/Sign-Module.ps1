...
function Find-CodeSigningCertificate {
    ...
    if ($Thumbprint) {
        $cert = Get-ChildItem Cert:\CurrentUser\My | Where-Object { $_.Thumbprint -eq $Thumbprint -and $_.HasPrivateKey }
        if (-not $cert) { throw "Certificate with thumbprint '$Thumbprint' not found or missing private key" }
        return $cert
    }

    # Auto-discover: find code signing cert with "STkrbKeytrab" in subject
    $certs = Get-ChildItem Cert:\CurrentUser\My | Where-Object {
        ...
        $_.Subject -like "*STkrbKeytab*" -and 
        $_.HasPrivateKey -and
        $_.NotAfter -gt (Get-Date)
    }

    if ($certs.Count -eq 0) { throw "No valid STkrbKeytab code signing certificates found" }
    if ($certs.Count -gt 1) { 
